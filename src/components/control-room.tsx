...
import { Activity, ArrowLeft, Bot, Check, CircleAlert, CircleStop, LoaderCircle, LockKeyhole, Radio, RefreshCw, Send, ShieldCheck, Wallet, X } from "lucide-react";
...
export function ControlRoom() {
  ...
  async function connectWallet() {
    ...
    try {
      ...
      setMessage("Wallet connected to the devnet control room.");
    } catch {
      setStatus("error");
  ...
  async function submitReceipt() {
    if (!decision.allowed) {
      setStatus("error");
      setMessage("Blocked intents cannot produce an execution receipt.");
      return;
    }
    if (!window.solana?.publicKey) { await connectWallet(); return; }
    setStatus("submitting");
    setMessage("Writing a policy receipt to Solana devnet.");
    try {
      const connection = new Connection(process.env.NEXT_PUBLIC_SOLANA_RPC_URL ?? "https://api.devnet.solana.com", "confirmed");
      const data = Buffer.from(`VEYLOCK|${decision.mode}|${proposal.side}|${proposal.asset}|${proposal.amountUsd}|${Date.now()}`, "utf8");
      const instruction = new TransactionInstruction({
        keys: [{ pubkey: window.solana.publicKey, isSigner: true, isWritable: false }],
      ...
      setStatus("confirmed");
      setMessage("Receipt confirmed on Solana devnet.");
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "Devnet submission failed");
    }
  }
      ...
      <header className="border-b border-white/10">
        <div className="mx-auto flex min-h-16 max-w-[1500px] items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex items-center gap-4">
            ...
            <div><p className="font-semibold tracking-[-0.03em]">Veylock control room</p><p className="font-mono text-[10px] uppercase tracking-widest text-white/45">Devnet · paper mandate</p></div>
          </div>
          <button onClick={connectWallet} className="focus-ring inline-flex min-h-10 items-center gap-2 border border-white/15 px-4 text-sm font-semibold hover:bg-white/5"><Wallet size={16} aria-hidden="true" />{walletAddress ? compactAddress(walletAddress) : "Connect wallet"}</button>
        </div>
      </header>

      <div className="mx-auto grid max-w-[1500px] gap-px bg-white/10 lg:grid-cols-[310px_1fr_360px]">
        <aside className="bg-[#111713] p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-white/50">Authority and agent keys are separated. Changes require the owner signer.</p>
          <div className="mt-8 space-y-6">
            <PolicyRange label="Max action" value={policy.maxTradeUsd} min={100} max={1500} prefix="$" onChange={(value) => setPolicy({ ...policy, maxTradeUsd: value })} />
            <PolicyRange label="Daily budget" value={policy.dailyBudgetUsd} min={500} max={5000} step={100} prefix="$" onChange={(value) => setPolicy({ ...policy, dailyBudgetUsd: value })} />
